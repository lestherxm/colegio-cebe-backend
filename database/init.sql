...
CREATE TRIGGER TR_INSERT_ALUMNOS AFTER INSERT ON alumnos
FOR EACH ROW
...
Language plpgsql;
--* EL TRIGER EJECUTA ESA FUNCIONA DESPUES DE HABER INSERTADO UN ALUMNO
CREATE TRIGGER TR_UPDATE_ALUMNOS AFTER UPDATE ON alumnos
FOR EACH ROW
EXECUTE PROCEDURE fn_inscritos_aula_update();

--* FUNCION A UTILIZAR AL MOMENTO DE ELIMINAR LOS DATOS DE UN ALUMNO, SE TIENE QUE ACTUALIZAR LA CANTIDAD DE INSCRITOS EN EL AULA EN QUE SE ENCONTRABA EL ESTUDIANTE.
CREATE OR REPLACE FUNCTION fn_inscritos_aula_delete() RETURNS TRIGGER
AS
$$
BEGIN
    UPDATE aulas SET n_inscritos= (SELECT COUNT(id_aula) FROM alumnos WHERE id_aula = old.id_aula) WHERE id_aula = old.id_aula;
...
Language plpgsql;
--* EL TRIGER EJECUTA ESA FUNCIONA DESPUES DE HABER INSERTADO UN ALUMNO
CREATE TRIGGER TR_DELETE_ALUMNOS AFTER DELETE ON alumnos
FOR EACH ROW
EXECUTE PROCEDURE fn_inscritos_aula_delete();
